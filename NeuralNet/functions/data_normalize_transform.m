...
%     end
    ...
    tags = '_normalized';
    imwrite(image, fullfile(normalizeImagePath,sprintf('%s%s%s',name,tags,'.jpg')));
    ...
    imwrite(warpImage, fullfile(normalizeImagePath,sprintf('%s%s%s',name,tags,'.jpg')));
    imwrite(warpLabel, fullfile(normalizePixelLabelPath,sprintf('%s%s%s',name,tags,'.png')));
    figure(1);
    imshow(warpImage);
    pause(0.01);
    ...
    rotateImage = imresize(imrotate(image, rotateAngle), [numrows numcols]);
    rotateLabel = imresize(imrotate(label, rotateAngle), [numrows numcols]);
