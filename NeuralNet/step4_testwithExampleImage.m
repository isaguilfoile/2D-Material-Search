...
dataPath = fullfile(pwd, 'fullTrain/');
...
for iter = 1:length(imageExtensions)
    imageList = [imageList;...
        getAllFiles(imagePath, sprintf('*_norm%s',imageExtensions{iter}), 0)];
end

imageListOriginal = {};
for iter = 1:length(imageExtensions)
    imageListOriginal = [imageListOriginal;...
        getAllFiles(imagePath, sprintf('*_orig%s',imageExtensions{iter}), 0)];
end

%% read the net files

netName = 'newTrain.mat';  % load the trained SegNet from current directory
