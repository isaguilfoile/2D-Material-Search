...
% if you desire to train images of your own, you can put the images there
dataPath = fullfile(pwd, 'fullTrain/Augmented_data/');
imageFolder = 'images';
labelFolder = 'labels';
...
validData = combine(imdsValid, pxdsValid);
...
opts = trainingOptions('sgdm',...
    ...
    'MaxEpochs', 10,...  % max epochs, default = 10
    'MiniBatchSize',16,...
    'Shuffle', 'every-epoch', ...
...
save(sprintf('newTrain.mat',depth, classWeights(1), classWeights(2), classWeights(3)), 'net', 'traininfo', 'layers','classWeights');
...
for p = 1:numValid
    index = ceil(rand() * min(numel(imdsValid.Files), numel(pxdsResults.Files)));
